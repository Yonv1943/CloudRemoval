...
def get_feed_queue(feed_queue):  # model_train
    ...
    loss_gene = tf.reduce_mean(fake_disc)
    loss_disc = tf.reduce_mean(real_disc) - loss_gene
    epsilon = tf.random_uniform([], 0.0, 1.0)
    pena_ans = epsilon * inp_ground + (1 - epsilon) * out_ground
    pena_disc = discriminator(pena_ans, 2 ** 6, disc_name, reuse=True)

    pena_gradient = tf.gradients(pena_disc, pena_ans)
    pena_slopes = tf.sqrt(tf.reduce_sum(tf.square(pena_gradient), axis=1))
    pena = tf.reduce_mean(tf.square(pena_slopes - 1.0))
    loss_disc += pena  # scale?
