...
def process_train(feed_queue, buff_queue):
    ...
    eval_fetch = [ten_ground, out_ground, ten_cloud3, ten_patch3, disc_fake3]
    eval_feed_dict = {inp_ground: eval_list[0][:C.eval_size],
                      inp_cloud1: eval_list[1][:C.eval_size]}
...
def process_feed(feed_queue, buff_queue):
    ...
    feed_queue.put([np.concatenate((grounds[eval_id], grounds[eval_id]), axis=0),
                    np.concatenate((cloud1s[eval_id], cloud1s[eval_id]), axis=0)])  # for eval
    ...
    try:
        for epoch in range(C.train_epoch):
            ...
            for i0, i1 in zip(i0_range, i1_range):
                ...
                int_cloud1 = cloud1s[j:j + bs]
                int_cloud1[int_cloud1 < threshold] = 0
                int_cloud1[int_cloud1 >= threshold] = 255

                int_grounds = np.concatenate((grounds[j:j + bs],
                                              grounds_buff[j:j + bs]), axis=0)
                int_cloud1s = np.concatenate((int_cloud1,
                                              cloud1s_buff[j:j + bs]), axis=0)
...
def run():
    ...
    feed_queue = mp.Queue(maxsize=16)
    buff_queue = mp.Queue(maxsize=16)
