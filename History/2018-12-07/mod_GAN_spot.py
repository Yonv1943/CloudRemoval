...
def process_train(feed_queue, buff_queue):  # model_train
    ...
    ten_mask01 = ten_cloud1 * tf.ones([1, 1, 1, 3], tf.float32)
    ten_mask10 = 1.0 - ten_mask01

    ten_ragged = inp_aerial * ten_mask10
    ten_aerial = tf.concat([ten_ragged, ten_cloud1], axis=3)
...
def process_data(feed_queue, buff_queue):
    data_aerial = img_util.get_data__aerial(C.train_size, channel=3)
    data_buffer = img_util.get_data__buffer(C.train_size, channel=3)
    data_cloud1 = img_util.get_data__cloud1(C.train_size)
    print("||Data_sets: ready for check")

    rd.shuffle(data_aerial)
    eval_id = min(105, C.train_size - C.batch_size)
    feed_queue.put([data_aerial[eval_id:eval_id + C.eval_size],
                    data_cloud1[eval_id:eval_id + C.eval_size], ])  # for eval
    feed_queue.put([data_aerial[:C.batch_size],
                    data_buffer[:C.batch_size],
                    data_cloud1[:C.batch_size], ])  # for check

    try:
        print("||Data_sets: ready for training")
        for epoch in range(C.train_epoch):
            if epoch % 8 == 0:  # np.rot90()
                data_aerial[:C.batch_size] = data_aerial[:C.batch_size].transpose((0, 2, 1, 3))
                data_buffer[:C.batch_size] = data_buffer[:C.batch_size].transpose((0, 2, 1, 3))
            elif epoch % 4 == 3:
                data_cloud1[:C.batch_size] = data_cloud1[:C.batch_size].transpose((0, 2, 1, 3))
            else:  # shuffle mildly
                rd_j, rd_k = rd.randint(0, int(C.train_size * 0.5 - C.batch_size), size=2)
                rd_k += int(C.train_size * 0.5)

                data_aerial[rd_j:rd_j + C.batch_size], data_aerial[rd_k:rd_k + C.batch_size] = \
                    data_aerial[rd_k:rd_k + C.batch_size], data_aerial[rd_j:rd_j + C.batch_size]
                data_buffer[rd_j:rd_j + C.batch_size], data_buffer[rd_k:rd_k + C.batch_size] = \
                    data_buffer[rd_k:rd_k + C.batch_size], data_buffer[rd_j:rd_j + C.batch_size]

            for i in range(C.batch_epoch):
                j = i * C.batch_size
                feed_queue.put([
                    data_aerial[j: j + C.batch_size],
                    data_buffer[j: j + C.batch_size],
                ])

                while buff_queue.qsize() > 0:
                    k, buff_get = buff_queue.get()
                    replace_idxs = rd.randint(0, C.batch_size, C.replace_num)
...
def run():  # beta
    ...
    [setattr(p, 'deamon', True) for p in process]
