...
def process_train(feed_queue, buff_queue):  # model_train
    ...
    inp_aerial = tf.placeholder(tf.float32, [None, C.size, C.size, 3], name='inp_aerial')
    inp_buffer = tf.placeholder(tf.float32, [None, C.size, C.size, 3], name='inp_buffer')
    inp_cloud1 = tf.placeholder(tf.float32, [None, C.size, C.size, 1], name='inp_cloud1')
    ...
    ten_patch3 = generator(ten_aerial, dim=24, name=gene_name, reuse=False)

    '''loss, optz'''
    inp_patch3 = inp_aerial * ten_mask01
    out_patch3 = ten_patch3 * ten_mask01

    out_aerial = ten_ragged + out_patch3
    buf_aerial = ten_ragged + inp_buffer

    real_disc = discriminator(inp_aerial, inp_patch3, 24, disc_name, reuse=False)
    buff_disc = discriminator(buf_aerial, inp_buffer, 24, disc_name, reuse=True)
    fake_disc = discriminator(out_aerial, out_patch3, 24, disc_name, reuse=True)
    ...
    feed_dict = dict()
    eval_out_fetch = [inp_aerial, out_aerial, ten_patch3, ten_mask01]
    ...
    try:
        for epoch in range(C.train_epoch):
            ...
            for i in range(C.batch_epoch):
                ...
                buff_put, batch_loss, _ = sess.run([out_patch3, loss, optz], feed_dict)
...
def process_data(feed_queue, buff_queue):
    ...
    data_cloud1 = img_util.get_data__circle(bs)
